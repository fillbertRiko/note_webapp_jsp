...
//		System.out.println(doc.toString());
		...
		return new Post(doc.getObjectId("_id").toHexString(), doc.getString("userId"), doc.getString("topicId"),
				doc.getString("title"), doc.getString("content"), doc.getDate("createdAt"), doc.getDate("updatedAt"),
				doc.getString("accessLevelId"), doc.getString("allowComment"), doc.getList("allowViewer", String.class),
				doc.getDate("month"), doc.getDate("year"));
	}
	...
	public boolean createPost(Post post) {
		...
		Document postDoc = new Document("_id", newId).append("userId", post.getUserId())
				.append("topicId", post.getTopicId()).append("title", post.getTitle())
				.append("content", post.getContent()).append("createdAt", post.getTimeCreate())
				.append("updatedAt", post.getTimeUpdate()).append("accessLevelId", post.getAccessLevelId())
				.append("allowViewer", post.getAllowViewer()).append("allowComment", post.getNumberAllowComment());
		try {
			posts.insertOne(postDoc);
			post.setId(newId.toHexString());
			System.out.println(
					"Post created with user ID: " + post.getUserId() + "\n And post have title: " + post.getTitle());
			return true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Post> findMyPost(String currentUserId, int page) {
		...
		try (MongoCursor<Document> cursor = posts.find(filter).sort(new Document("createdAt", -1)).skip(skipValue)
	...
	public Post findPostByUserId(String postId, String currentUserId) {
		try {
			Bson filter = Filters.and(Filters.eq("_id", new ObjectId(postId)), Filters.eq("userId", currentUserId));
	...
	public List<Post> findByMonth(String userId, int month, int year) {
		...
		Bson filter = Filters.and(Filters.eq("userId", userId), Filters.gte("createdAt", startDate),
	...
	public List<Post> findPostsVisibleToUser(String ownerId, List<String> allowAccessLevels, int page, String viewerId,
		...
		Bson filterAccess = Filters.and(Filters.eq("userId", ownerId),
				Filters.or(Filters.in("accessLevelId", allowAccessLevels),
						Filters.and(Filters.eq("accessLevelId", "PROTECTED_1"), Filters.eq("allowViewer", viewerId))));

		Bson searchFilter = new Document();

		if (keyword != null && !keyword.trim().isEmpty()) {
			searchFilter = Filters.or(Filters.regex("title", ".*" + java.util.regex.Pattern.quote(keyword) + ".*", "i"),
		...
		try (MongoCursor<Document> cursor = posts.find(filterAccess).sort(new Document("createdAt", -1)).skip(skipValue)
				.limit(pageSize).iterator()) {
			while (cursor.hasNext()) {
				Document doc = cursor.next();
	...
	public boolean updatePost(Post post) {
		...
		try {
			Bson filter = Filters.and(Filters.eq("_id", new ObjectId(post.getId())),
